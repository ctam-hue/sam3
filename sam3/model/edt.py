...
# Try to import triton - it's optional for GPU acceleration
try:
    import triton
...
# Only define triton kernels if triton is available
if TRITON_AVAILABLE:
    @triton.jit
    ...
    def edt_triton(data: torch.Tensor):
        """
        Fallback EDT implementation using PyTorch when triton is not available.
        ...
        import warnings
        warnings.warn(
            "Triton is not available. Using slower PyTorch fallback for EDT. "
            "Install triton for better performance: pip install triton",
            UserWarning
        )
        
        # Simple O(N^3) implementation using PyTorch
        assert data.dim() == 3
        B, H, W = data.shape
        ...
        for b in range(B):
            ...
            zero_positions = torch.nonzero(~mask.bool(), as_tuple=False)
            ...
            # For each pixel, compute distance to nearest zero
            ...
            # Compute distances to all zero pixels
            min_dist = torch.full((H, W), float('inf'), device=data.device)
            for zy, zx in zero_positions:
                dist = torch.sqrt(
                    (y_coords - zy.float()) ** 2 + (x_coords - zx.float()) ** 2
                )
