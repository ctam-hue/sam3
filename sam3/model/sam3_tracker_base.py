...
class Sam3TrackerBase(torch.nn.Module):
    ...
    def _get_tpos_enc(self, rel_pos_list, device, max_abs_pos=None, dummy=False):
        ...
        pos_tensor = torch.tensor(rel_pos_list)
        if device.type == "cuda":
            pos_tensor = pos_tensor.pin_memory().to(device=device, non_blocking=True)
        else:
            pos_tensor = pos_tensor.to(device=device)
        pos_enc = pos_tensor / t_diff_max
